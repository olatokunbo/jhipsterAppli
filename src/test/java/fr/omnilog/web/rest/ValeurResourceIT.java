...
class ValeurResourceIT {
    ...
    void createValeur() throws Exception {
        ...
        // Create the Valeur
        ValeurDTO valeurDTO = valeurMapper.toDto(valeur);
        restValeurMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(valeurDTO)))
    ...
    void putExistingValeur() throws Exception {
        ...
        updatedValeur
            ...
            .abreviation(UPDATED_ABREVIATION)
            .description(UPDATED_DESCRIPTION);
        ValeurDTO valeurDTO = valeurMapper.toDto(updatedValeur);

        restValeurMockMvc
            .perform(
                put(ENTITY_API_URL_ID, valeurDTO.getId())
    ...
    void putWithMissingIdPathParamValeur() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restValeurMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(valeurDTO)))
    ...
    void partialUpdateValeurWithPatch() throws Exception {
        ...
        partialUpdatedValeur.code(UPDATED_CODE).ordre(UPDATED_ORDRE);
    ...
    void patchNonExistingValeur() throws Exception {
        ...
        restValeurMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, valeurDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(valeurDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamValeur() throws Exception {
        ...
        restValeurMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(valeurDTO))
            )
            .andExpect(status().isMethodNotAllowed());
