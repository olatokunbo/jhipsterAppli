...
describe('Valeur Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call Valeur query and add missing value', () => {
      ...
      const valeurParent: IValeur = { id: 10630 };
      valeur.valeurParent = valeurParent;

      const valeurCollection: IValeur[] = [{ id: 27087 }];
      jest.spyOn(valeurService, 'query').mockReturnValue(of(new HttpResponse({ body: valeurCollection })));
      const additionalValeurs = [valeurParent];
    ...
    it('Should update editForm', () => {
      const valeur: IValeur = { id: 456 };
      const valeurParent: IValeur = { id: 11111 };
      valeur.valeurParent = valeurParent;

      activatedRoute.data = of({ valeur });
      comp.ngOnInit();

      expect(comp.valeursSharedCollection).toContain(valeurParent);
